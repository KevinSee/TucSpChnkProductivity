...
```{r redds-smolts, fig.height = 4, fig.width = 4, fig.cap = "Plot of redds vs. smolts, colored by year."}
# color
# redd_smolt_p <- 
#   mod_df |>
#   ggplot(aes(x = number_redds,
#              y = number_smolts)) +
#   # geom_smooth(data = mod_df |>
#   #               filter(brood_yr < 2007),
#   #             method = lm,
#   #             se = T,
#   #             alpha = 0.3,
#   #             fill = "lightpink",
...
#   #             fill = "lightblue",
#   #             color = "blue") +
#   geom_point(aes(color = smolts_redd),
#              size = 3) +
#   geom_text_repel(aes(label = brood_yr),
#                   max.overlaps = 40,
#                   color = "gray50") +
#   scale_color_continuous_sequential("Hawaii",
#                                     name = "Smolts / redd") +
...
redd_smolt_p <- 
  ...
  #             fill = "gray70") +
  geom_point(aes(color = emigration_yr),
             size = 3) +
  scale_color_continuous_sequential("Hawaii",
                                    name = "Smolt Year") +
  theme(legend.position = "bottom") +
...
#   ggplot(aes(x = number_redds,
#              y = number_smolts)) +
#   geom_point(aes(fill = smolts_redd),
#              color = "gray70",
#              shape = 21,
#              size = 3) +
#   geom_text_repel(aes(label = brood_yr),
...
init_form <-
  formula(cbind(suc_w_jacks,
            ...
            s(phos, k = 4, bs = "tp", m = 2) +
            # phos:stray_prop +
            # s(mean_rkm, k = 4, bs = "tp") +
            # s(sd_rkm, k = 4, bs = "tp") +
            # s(mean_rkm, phos, k = 4) +
            # s(mean_smolt_length, mean_rkm, k = 4) +
            # s(mean_smolt_length, phos, k = 4) +
...
```{r glm-fits, eval = F, echo = F}
library(lme4)
glm_mod_2a <-
  glm(cbind(suc_w_jacks,
              number_smolts - suc_w_jacks) ~
          mean_smolt_length +
          phos +
          mean_rkm,
...
PseudoR2(glm_mod_2b,
         # which = "McFaddenAdj")
         which = "McKelveyZavoina")
# which = "VeallZimmermann")
# which = "AldrichNelson")
```
...
# predict_gam(gam_mod_1,
#             length_out = 50,
#             # values = list(phos = median(mod_df_1$phos)),
#             series = c("mean_rkm", "sd_rkm"),
#             tran_fun = exp) |>
#   ggplot(aes(x = mean_rkm,
#              y = sd_rkm,
#              fill = smolts_redd)) +
...
#                                   name = "Smolts/Redd") +
#   theme(legend.position = "bottom") +
#   labs(x = "Mean Redd RKM",
#        y = "SD Redd RKM")

```

```{r mod-1-plots, eval = F}
sm <-
  smooth_estimates(gam_mod_1) |> 
  add_confint() |> 
  pivot_longer(cols = mean_rkm:emigration_yr,
               names_to = "covar",
               values_to = "value") |> 
  filter(!is.na(value))
...
res <-
  mod_df_1 |> 
  select(-median_rkm) |> 
  add_partial_residuals(gam_mod_1) |> 
  pivot_longer(cols = c(emigration_yr,
                        redds,
                        mean_rkm,
                        sd_rkm,
                        phos,
  ...
  mutate(covar = name,
         ...
         across(covar,
                ~ str_remove(., "\\)$")),
         type = if_else(str_detect(name, "^s\\("),
                        "resid",
                        "obs")) |> 
  select(-name) |> 
  pivot_wider(names_from = type,
              values_from = value) |> 
  filter(covar %in% unique(sm$covar))
...
by_p2 <-
  predict_gam(gam_mod_2c,
            length_out = diff(range(mod_df_2$brood_yr)),
            series = "brood_yr",
            # values = list(brood_yr = unique(mod_df_2$brood_yr)),
            tran_fun = exp) |>
  rename(rps = total_rps) |> 
  mutate(model = "Total") |> 
  bind_rows(predict_gam(gam_mod_2d,
              ...
              rename(rps = adult_rps) |> 
              mutate(model = "Adult")) |> 
  # filter(brood_yr %in% unique(mod_df_2$brood_yr)) |> 
  ggplot(aes(x = brood_yr,
             y = rps,
             linetype = model,
             color = model,
...
# predict_gam(gam_mod_2c,
#             series = "redds",
#             values = list(redds = seq(0, 600, by = 5)),
#             tran_fun = exp) |>
#   rename(rps = total_rps) |>
#   mutate(model = "Total") |>
#   bind_rows(predict_gam(gam_mod_2d,
...
#               rename(rps = adult_rps) |>
#               mutate(model = "Adult")) |>
#   mutate(recruits = rps * redds,
#          across(ends_with("ci"),
#                 ~ . * redds)) |>
#   group_by(model) |>
#   filter(recruits == max(recruits)) |>
#   ungroup()
